...
<main id="login-page">
    ...
    {#if emailSent}
        <form on:submit|preventDefault={sendPassword} id="form">
            ...
            <Input type="text" label="Token" bind:value={token} error="token" required size=40 />
            <br>
            <Input type="password" label="Senha" bind:value={password} error="password" required size=40 />
            <br>
            <Input type="password" label="Confirme sua senha" bind:value={password_confirmation} error="password_confirmation" required size=40 />
        </form>
    {:else}
