<script>import userIcon from "./icons/edit-user.svg?raw";
...
import { confirmation, ConfirmationComponent } from "../confirmation";
import { beforeNavigate } from "$app/navigation";
import { slide } from "svelte/transition";
async function logout() {
  const response = await Ajax.delete("/auth").send("none");
  if (response.error)
    return;
  user.clean();
  goto("/login");
